...
if __name__ == '__main__':
    ...
    with h5py.File(in_h5py_path, 'r+') as f_in:
        for speaker in f_in['train'].keys():
            ...
            f_in[f'train/{speaker}'].attrs['f0_mean'] = f0_mean
            f_in[f'train/{speaker}'].attrs['f0_std'] = f0_std
            # processing MCEP
            ...
            f_in[f'train/{speaker}'].attrs['mc_mean'] = mc_mean
            f_in[f'train/{speaker}'].attrs['mc_std'] = mc_std

        for speaker in f_in['train'].keys():
            print(f'normalize speaker_id={speaker}')
            mc_mean = f_in[f'train/{speaker}'].attrs['mc_mean']
            mc_std = f_in[f'train/{speaker}'].attrs['mc_std']
