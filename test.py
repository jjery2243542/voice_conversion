...
if __name__ == '__main__':
    ...
    solver.load_model('/storage/model/voice_conversion/vctk/clf/norm/8_speakers_1e-3.pkl-100000')
        ...
        if one == True:
            ...
            for filename in glob.glob(os.path.join(directory, '*.wav')):
                ...
                if spec_tensor.size(1) > 1000:
                    ...
                    print(result.shape)
                    wav_data = spectrogram2wav(result)
                write(os.path.join('./for_VC/Jeff_dean/result4/', sub_filename), rate=16000, data=wav_data)
